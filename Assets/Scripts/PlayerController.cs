...
public class PlayerController : NetworkBehaviour
{
    ...
    private void OnCollisionExit(Collision collidedAsset)
    {
        if (collidedAsset.gameObject.CompareTag("DestructibleScenery"))
        {
            AbleToDestroy = false;
            AssetToDestroy = null;
            if (collidedAsset.gameObject.GetComponent<DestructibleAttributes>().ResourceUsed == true)
            {
                collidedAsset.gameObject.GetComponent<DestructibleAttributes>().LocalDestroyAsset();
            }
        }
        if (collidedAsset.gameObject.CompareTag("LootableContainer"))
        {
